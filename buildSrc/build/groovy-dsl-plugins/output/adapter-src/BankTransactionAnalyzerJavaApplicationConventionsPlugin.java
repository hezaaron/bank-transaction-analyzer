...
/**
 * Precompiled bank.transaction.analyzer.java-application-conventions script plugin.
 **/
public class BankTransactionAnalyzerJavaApplicationConventionsPlugin implements org.gradle.api.Plugin<org.gradle.api.internal.project.ProjectInternal> {
    ...
    public void apply(org.gradle.api.internal.project.ProjectInternal target) {
        assertSupportedByCurrentGradleVersion();
                target.getPluginManager().apply("bank.transaction.analyzer.java-common-conventions");
        target.getPluginManager().apply("application");

        try {
            Class<? extends BasicScript> precompiledScriptClass = Class.forName("precompiled_BankTransactionAnalyzerJavaApplicationConventions").asSubclass(BasicScript.class);
